...
#include "Filas.h"
    ...
    #define Filas_SetP4PinDriveMode(shift, mode)  \
    do { \
        Filas_PC =   (Filas_PC & \
                                (uint32)(~(uint32)(Filas_DRIVE_MODE_IND_MASK << \
                                (Filas_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Filas_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Filas_SetP4PinDriveMode(shift, mode)  \
        do { \
            Filas_USBIO_CTRL_REG = (Filas_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Filas_DRIVE_MODE_IND_MASK << \
                                    (Filas_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Filas_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Filas_SetP4PinDriveMode(Filas__0__SHIFT, mode);
		Filas_SetP4PinDriveMode(Filas__1__SHIFT, mode);
		Filas_SetP4PinDriveMode(Filas__2__SHIFT, mode);
		Filas_SetP4PinDriveMode(Filas__3__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Filas_DR & (uint8)(~Filas_MASK));
    drVal = (drVal | ((uint8)(value << Filas_SHIFT) & Filas_MASK));
    Filas_DR = (uint32)drVal;
}
...
{
    return (uint8)((Filas_PS & Filas_MASK) >> Filas_SHIFT);
}
...
{
    return (uint8)((Filas_DR & Filas_MASK) >> Filas_SHIFT);
}
...
void Filas_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Filas_INTCFG & (uint32)(~(uint32)position);
    Filas_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Filas_INTSTAT & Filas_MASK);
	Filas_INTSTAT = maskedStatus;
    return maskedStatus >> Filas_SHIFT;
}
