...
#include "Col.h"
...
#if defined(Col__PC)
    #define Col_SetP4PinDriveMode(shift, mode)  \
    do { \
        Col_PC =   (Col_PC & \
                                (uint32)(~(uint32)(Col_DRIVE_MODE_IND_MASK << \
                                (Col_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Col_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Col_SetP4PinDriveMode(shift, mode)  \
        do { \
            Col_USBIO_CTRL_REG = (Col_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Col_DRIVE_MODE_IND_MASK << \
                                    (Col_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Col_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Col_SetP4PinDriveMode(Col__0__SHIFT, mode);
		Col_SetP4PinDriveMode(Col__1__SHIFT, mode);
		Col_SetP4PinDriveMode(Col__2__SHIFT, mode);
		Col_SetP4PinDriveMode(Col__3__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Col_DR & (uint8)(~Col_MASK));
    drVal = (drVal | ((uint8)(value << Col_SHIFT) & Col_MASK));
    Col_DR = (uint32)drVal;
}
...
uint8 Col_Read(void)
{
    return (uint8)((Col_PS & Col_MASK) >> Col_SHIFT);
}
...
{
    return (uint8)((Col_DR & Col_MASK) >> Col_SHIFT);
}
...
void Col_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Col_INTCFG & (uint32)(~(uint32)position);
    Col_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Col_INTSTAT & Col_MASK);
	Col_INTSTAT = maskedStatus;
    return maskedStatus >> Col_SHIFT;
}
